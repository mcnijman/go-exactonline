...
func TestTasksEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/activities/Tasks?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/activities/Tasks?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Tasks.List(context.Background(), 0, true)
	if err != nil {
...
func TestTasksEndpoint_List(t *testing.T) {
	...
	entities, err := s.Tasks.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("TasksEndpoint.List returned error: %v", err)
	}

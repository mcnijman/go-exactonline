...
func TestComplaintsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/activities/Complaints?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/activities/Complaints?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Complaints.List(context.Background(), 0, true)
	if err != nil {
...
func TestComplaintsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Complaints.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("ComplaintsEndpoint.List returned error: %v", err)
	}

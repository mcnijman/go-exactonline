...
func TestWorkcentersEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/manufacturing/Workcenters?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/manufacturing/Workcenters?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Workcenters.List(context.Background(), 0, true)
	if err != nil {
...
func TestWorkcentersEndpoint_List(t *testing.T) {
	...
	entities, err := s.Workcenters.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("WorkcentersEndpoint.List returned error: %v", err)
	}

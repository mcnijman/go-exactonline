...
func TestOperationsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/manufacturing/Operations?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/manufacturing/Operations?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Operations.List(context.Background(), 0, true)
	if err != nil {
...
func TestOperationsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Operations.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("OperationsEndpoint.List returned error: %v", err)
	}

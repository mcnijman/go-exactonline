...
func TestByProductReceiptsEndpoint_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			&ByProductReceipts{StockTransactionId: s1, MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}
...
func TestByProductReceiptsEndpoint_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"1",
			args{context.Background(), 0, &ByProductReceipts{MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}}},
			&ByProductReceipts{MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}
	for _, tt := range tests {

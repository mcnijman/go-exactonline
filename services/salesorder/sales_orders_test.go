...
func TestSalesOrdersEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/salesorder/SalesOrders?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/salesorder/SalesOrders?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.SalesOrders.List(context.Background(), 0, true)
	if err != nil {
...
func TestSalesOrdersEndpoint_List(t *testing.T) {
	...
	entities, err := s.SalesOrders.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("SalesOrdersEndpoint.List returned error: %v", err)
	}

...
func TestVATCodesEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/vat/VATCodes?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/vat/VATCodes?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.VATCodes.List(context.Background(), 0, true)
	if err != nil {
...
func TestVATCodesEndpoint_List(t *testing.T) {
	...
	entities, err := s.VATCodes.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("VATCodesEndpoint.List returned error: %v", err)
	}

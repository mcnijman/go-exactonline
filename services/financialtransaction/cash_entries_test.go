...
func TestCashEntriesEndpoint_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			&CashEntries{EntryID: s1, MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}
...
func TestCashEntriesEndpoint_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"1",
			args{context.Background(), 0, &CashEntries{MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}}},
			&CashEntries{MetaData: &api.MetaData{URI: &types.URL{&url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}
	for _, tt := range tests {

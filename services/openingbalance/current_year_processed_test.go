...
func TestCurrentYearProcessedEndpoint_List_all(t *testing.T) {
	...
	entities, err := s.CurrentYearProcessed.List(context.Background(), 0, true, opts1)
	if err != nil {
...
func TestCurrentYearProcessedEndpoint_List(t *testing.T) {
	s, mux, _, teardown := setup()
	defer teardown()
	...
	entities, err := s.CurrentYearProcessed.List(context.Background(), 0, false, opts1)
	if err != nil {
		t.Errorf("CurrentYearProcessedEndpoint.List returned error: %v", err)
	}
...
func TestCurrentYearProcessedEndpoint_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			&CurrentYearProcessed{Division: s1, MetaData: &api.MetaData{URI: &types.URL{URL: &url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}

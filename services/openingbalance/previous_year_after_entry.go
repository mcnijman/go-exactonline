...
func (s *PreviousYearAfterEntryEndpoint) List(ctx context.Context, division int, all bool) ([]*PreviousYearAfterEntry, error) {
	var entities []*PreviousYearAfterEntry
	u, _ := s.client.ResolvePathWithDivision("/api/v1/{division}/openingbalance/PreviousYear/AfterEntry?$select=*", division) // #nosec
	if all {

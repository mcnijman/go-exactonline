...
func TestItemGroupsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/logistics/ItemGroups?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/logistics/ItemGroups?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.ItemGroups.List(context.Background(), 0, true)
	if err != nil {
...
func TestItemGroupsEndpoint_List(t *testing.T) {
	...
	entities, err := s.ItemGroups.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("ItemGroupsEndpoint.List returned error: %v", err)
	}

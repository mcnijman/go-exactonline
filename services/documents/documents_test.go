...
func TestDocumentsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/documents/Documents?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/documents/Documents?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Documents.List(context.Background(), 0, true)
	if err != nil {
...
func TestDocumentsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Documents.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("DocumentsEndpoint.List returned error: %v", err)
	}

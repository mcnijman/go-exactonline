...
func TestAccountsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/crm/Accounts?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/crm/Accounts?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Accounts.List(context.Background(), 0, true)
	if err != nil {
...
func TestAccountsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Accounts.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("AccountsEndpoint.List returned error: %v", err)
	}

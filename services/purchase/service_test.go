...
func TestNewPurchaseService(t *testing.T) {
	c := api.NewClient(nil)
	s := NewPurchaseService(c)

	if !reflect.DeepEqual(c, s.client) {
		t.Error("Clients are supposed to be the same")
	}

	if s.PurchaseInvoiceLines == nil {
		t.Error("Property PurchaseInvoiceLines should not be nil")
	}
	if s.PurchaseInvoices == nil {
		t.Error("Property PurchaseInvoices should not be nil")
	}
}
...
func setup() (service *PurchaseService, mux *http.ServeMux, serverURL string, teardown func()) {
	...
	client.BaseURL = url
	service = NewPurchaseService(client)

...
func TestAccountOwnersEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/accountancy/AccountOwners?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/accountancy/AccountOwners?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.AccountOwners.List(context.Background(), 0, true)
	if err != nil {
...
func TestAccountOwnersEndpoint_List(t *testing.T) {
	...
	entities, err := s.AccountOwners.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("AccountOwnersEndpoint.List returned error: %v", err)
	}

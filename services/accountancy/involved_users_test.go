...
func TestInvolvedUsersEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/accountancy/InvolvedUsers?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/accountancy/InvolvedUsers?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.InvolvedUsers.List(context.Background(), 0, true)
	if err != nil {
...
func TestInvolvedUsersEndpoint_List(t *testing.T) {
	...
	entities, err := s.InvolvedUsers.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("InvolvedUsersEndpoint.List returned error: %v", err)
	}

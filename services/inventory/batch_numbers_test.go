...
func TestBatchNumbersEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/inventory/BatchNumbers?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/inventory/BatchNumbers?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.BatchNumbers.List(context.Background(), 0, true)
	if err != nil {
...
func TestBatchNumbersEndpoint_List(t *testing.T) {
	...
	entities, err := s.BatchNumbers.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("BatchNumbersEndpoint.List returned error: %v", err)
	}

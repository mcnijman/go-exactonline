...
func TestAssetsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/assets/Assets?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/assets/Assets?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Assets.List(context.Background(), 0, true)
	if err != nil {
...
func TestAssetsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Assets.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("AssetsEndpoint.List returned error: %v", err)
	}

...
func TestInvoiceTermsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/project/InvoiceTerms?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/project/InvoiceTerms?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.InvoiceTerms.List(context.Background(), 0, true)
	if err != nil {
...
func TestInvoiceTermsEndpoint_List(t *testing.T) {
	...
	entities, err := s.InvoiceTerms.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("InvoiceTermsEndpoint.List returned error: %v", err)
	}

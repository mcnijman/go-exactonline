...
func TestProfitLossOverviewEndpoint_List_all(t *testing.T) {
	...
	entities, err := s.ProfitLossOverview.List(context.Background(), 0, true, opts1)
	if err != nil {
...
func TestProfitLossOverviewEndpoint_List(t *testing.T) {
	...
	entities, err := s.ProfitLossOverview.List(context.Background(), 0, false, opts1)
	if err != nil {
		t.Errorf("ProfitLossOverviewEndpoint.List returned error: %v", err)
	}
...
func TestProfitLossOverviewEndpoint_Get(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			&ProfitLossOverview{CurrentYear: s1, MetaData: &api.MetaData{URI: &types.URL{URL: &url.URL{Scheme: "https", Host: "start.exactonline.nl"}}}},
			false,
		},
	}

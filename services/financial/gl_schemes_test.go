...
func TestGLSchemesEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/financial/GLSchemes?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/financial/GLSchemes?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.GLSchemes.List(context.Background(), 0, true)
	if err != nil {
...
func TestGLSchemesEndpoint_List(t *testing.T) {
	...
	entities, err := s.GLSchemes.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("GLSchemesEndpoint.List returned error: %v", err)
	}

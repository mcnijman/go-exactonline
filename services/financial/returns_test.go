...
func TestReturnsEndpoint_List_all(t *testing.T) {
	...
	u, e := s.client.ResolvePathWithDivision("/api/v1/{division}/read/financial/Returns?$select=*", 0)
	if e != nil {
	...
	u2, e2 := s.client.ResolvePathWithDivision("/api/v1/{division}/read/financial/Returns?$skiptoken=foo", 0)
	if e2 != nil {
	...
	entities, err := s.Returns.List(context.Background(), 0, true)
	if err != nil {
...
func TestReturnsEndpoint_List(t *testing.T) {
	...
	entities, err := s.Returns.List(context.Background(), 0, false)
	if err != nil {
		t.Errorf("ReturnsEndpoint.List returned error: %v", err)
	}

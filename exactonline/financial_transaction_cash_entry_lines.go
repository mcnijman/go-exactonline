...
// FinancialTransactionCashEntryLinesService is responsible for communicating with
// the CashEntryLines endpoint of the FinancialTransaction service.
type FinancialTransactionCashEntryLinesService service

// FinancialTransactionCashEntryLines:
...
type FinancialTransactionCashEntryLines struct {
	// ID:
...
func (s *FinancialTransactionCashEntryLines) GetIdentifier() GUID {
	return *s.ID
}

// List the CashEntryLines entities in the provided divison.
// If all is true, all the paginated results are fetched; if false, list the first page.
func (s *FinancialTransactionCashEntryLinesService) List(ctx context.Context, division int, all bool) ([]*FinancialTransactionCashEntryLines, error) {
	var entities []*FinancialTransactionCashEntryLines
...
/* // Get the CashEntryLines enitity, by ID.
func (s *FinancialTransactionCashEntryLinesService) Get(ctx context.Context, division int, id GUID) (*FinancialTransactionCashEntryLines, error) {
	var entities []*FinancialTransactionCashEntryLines
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/financialtransaction/CashEntryLines?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d CashEntryLines entities, expected 1", len(entities))
	}

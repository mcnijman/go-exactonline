...
/* // Get the Schedules enitity, by ID.
func (s *HRMSchedulesService) Get(ctx context.Context, division int, id GUID) (*HRMSchedules, error) {
	var entities []*HRMSchedules
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/hrm/Schedules?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d Schedules entities, expected 1", len(entities))
	}

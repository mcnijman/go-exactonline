...
/* // Get the PreviousYearProcessed enitity, by Division.
func (s *OpeningBalancePreviousYearProcessedService) Get(ctx context.Context, division int, id int) (*OpeningBalancePreviousYearProcessed, error) {
	var entities []*OpeningBalancePreviousYearProcessed
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/openingbalance/PreviousYear/Processed?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d PreviousYearProcessed entities, expected 1", len(entities))
	}

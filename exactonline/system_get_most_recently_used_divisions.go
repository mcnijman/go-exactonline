...
// SystemGetMostRecentlyUsedDivisionsService is responsible for communicating with
// the GetMostRecentlyUsedDivisions endpoint of the System service.
type SystemGetMostRecentlyUsedDivisionsService service

// SystemGetMostRecentlyUsedDivisions:
// Service: System
...
type SystemGetMostRecentlyUsedDivisions struct {
	// Code: Primary key
...
func (s *SystemGetMostRecentlyUsedDivisions) GetIdentifier() int {
	return *s.Code
}

// List the GetMostRecentlyUsedDivisions entities in the provided divison.
// If all is true, all the paginated results are fetched; if false, list the first page.
func (s *SystemGetMostRecentlyUsedDivisionsService) List(ctx context.Context, division int, all bool) ([]*SystemGetMostRecentlyUsedDivisions, error) {
	var entities []*SystemGetMostRecentlyUsedDivisions
...
/* // Get the GetMostRecentlyUsedDivisions enitity, by Code.
func (s *SystemGetMostRecentlyUsedDivisionsService) Get(ctx context.Context, division int, id int) (*SystemGetMostRecentlyUsedDivisions, error) {
	var entities []*SystemGetMostRecentlyUsedDivisions
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/system/GetMostRecentlyUsedDivisions?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d GetMostRecentlyUsedDivisions entities, expected 1", len(entities))
	}

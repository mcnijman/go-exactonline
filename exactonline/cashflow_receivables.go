...
type CashflowReceivables struct {
	...
	AccountBankAccountID *GUID `json:"AccountBankAccountID,omitempty"`
	...
	DirectDebitMandate *GUID `json:"DirectDebitMandate,omitempty"`
	...
	DiscountDueDate *Date `json:"DiscountDueDate,omitempty"`
	...
	LastPaymentDate *Date `json:"LastPaymentDate,omitempty"`
	...
	ReceivableSelected *Date `json:"ReceivableSelected,omitempty"`

	// ReceivableSelector: User who selected the receivable to be collected.
	ReceivableSelector *GUID `json:"ReceivableSelector,omitempty"`
	...
	TransactionDueDate *Date `json:"TransactionDueDate,omitempty"`
	...
	TransactionID *GUID `json:"TransactionID,omitempty"`
...
/* // Get the Receivables enitity, by ID.
func (s *CashflowReceivablesService) Get(ctx context.Context, division int, id GUID) (*CashflowReceivables, error) {
	var entities []*CashflowReceivables
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/cashflow/Receivables?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d Receivables entities, expected 1", len(entities))
	}

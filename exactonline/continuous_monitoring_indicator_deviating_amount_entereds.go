...
/* // Get the IndicatorDeviatingAmountEntereds enitity, by ID.
func (s *ContinuousMonitoringIndicatorDeviatingAmountEnteredsService) Get(ctx context.Context, division int, id GUID) (*ContinuousMonitoringIndicatorDeviatingAmountEntereds, error) {
	var entities []*ContinuousMonitoringIndicatorDeviatingAmountEntereds
	u, err := s.client.ResolvePathWithDivision("/api/v1/beta/{division}/continuousmonitoring/IndicatorDeviatingAmountEntereds?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d IndicatorDeviatingAmountEntereds entities, expected 1", len(entities))
	}

...
/* // Get the ShopOrderReceipts enitity, by StockTransactionId.
func (s *ManufacturingShopOrderReceiptsService) Get(ctx context.Context, division int, id GUID) (*ManufacturingShopOrderReceipts, error) {
	var entities []*ManufacturingShopOrderReceipts
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/manufacturing/ShopOrderReceipts?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d ShopOrderReceipts entities, expected 1", len(entities))
	}

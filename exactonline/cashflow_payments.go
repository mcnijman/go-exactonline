...
type CashflowPayments struct {
	...
	PaymentSelected *Date `json:"PaymentSelected,omitempty"`

	// PaymentSelector: User who selected the payment to be paid.
	PaymentSelector *GUID `json:"PaymentSelector,omitempty"`
...
/* // Get the Payments enitity, by ID.
func (s *CashflowPaymentsService) Get(ctx context.Context, division int, id GUID) (*CashflowPayments, error) {
	var entities []*CashflowPayments
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/cashflow/Payments?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d Payments entities, expected 1", len(entities))
	}

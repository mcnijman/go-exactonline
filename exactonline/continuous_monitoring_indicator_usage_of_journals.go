...
type ContinuousMonitoringIndicatorUsageOfJournals struct {
	...
	Journal *GUID `json:"Journal,omitempty"`
...
/* // Get the IndicatorUsageOfJournals enitity, by ID.
func (s *ContinuousMonitoringIndicatorUsageOfJournalsService) Get(ctx context.Context, division int, id GUID) (*ContinuousMonitoringIndicatorUsageOfJournals, error) {
	var entities []*ContinuousMonitoringIndicatorUsageOfJournals
	u, err := s.client.ResolvePathWithDivision("/api/v1/beta/{division}/continuousmonitoring/IndicatorUsageOfJournals?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d IndicatorUsageOfJournals entities, expected 1", len(entities))
	}

...
/* // Get the GoodsReceipts enitity, by ID.
func (s *PurchaseOrderGoodsReceiptsService) Get(ctx context.Context, division int, id GUID) (*PurchaseOrderGoodsReceipts, error) {
	var entities []*PurchaseOrderGoodsReceipts
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/purchaseorder/GoodsReceipts?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d GoodsReceipts entities, expected 1", len(entities))
	}

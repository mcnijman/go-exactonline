...
/* // Get the TransactionLines enitity, by ID.
func (s *FinancialTransactionTransactionLinesService) Get(ctx context.Context, division int, id GUID) (*FinancialTransactionTransactionLines, error) {
	var entities []*FinancialTransactionTransactionLines
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/financialtransaction/TransactionLines?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d TransactionLines entities, expected 1", len(entities))
	}

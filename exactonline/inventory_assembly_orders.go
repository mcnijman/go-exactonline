...
type InventoryAssemblyOrders struct {
	...
	AssemblyDate *Date `json:"AssemblyDate,omitempty"`
...
/* // Get the AssemblyOrders enitity, by ID.
func (s *InventoryAssemblyOrdersService) Get(ctx context.Context, division int, id GUID) (*InventoryAssemblyOrders, error) {
	var entities []*InventoryAssemblyOrders
	u, err := s.client.ResolvePathWithDivision("/api/v1/{division}/inventory/AssemblyOrders?$select=*", division)
	if err != nil {
		return nil, err
	...
	if len(entities) != 1 {
		return nil, fmt.Errorf("Returned %d AssemblyOrders entities, expected 1", len(entities))
	}
